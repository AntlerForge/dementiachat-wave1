...
const APP_VERSION = "wave1-2026-04-02.32";
...
async function saveInformationBoardNow() {
  ...
  if (state.infoBoardSaveInFlight) {
    state.infoBoardSaveQueued = true;
    return;
  }
...
function enforceRoleLock() {
  ...
  // Keep Dad accounts or locked profiles in Dad view mode and hide the role picker.
  if (isAuthedDad() || isDadRoleLocked()) {
    state.role = "dad";
    roleSelect.value = "dad";
    localStorage.setItem(ROLE_KEY, "dad");
